...
import com.example.myapplication.domain.AccountInfo
...
class AccountViewModel(application: Application) : AndroidViewModel(application) {
    ...
    //
    // viewModelScope 说明：
    // viewModelScope 是 AndroidX 提供的协程作用域，绑定到 ViewModel 的生命周期。
    // 当 ViewModel 被销毁时，viewModelScope 会自动取消所有协程，无需手动管理。
    // viewModelScope 默认在 Main 调度器上运行，Repository 方法内部使用 withContext(Dispatchers.IO)
    // 来确保数据库操作在 IO 线程执行。
     ...
     *
     * 执行流程：
     * 1. 使用 viewModelScope 启动协程（在 Main 调度器上运行）
     * 2. 调用 Repository 检查是否已登录（Repository 内部切换到 IO 线程）
     * 3. 如果已登录，获取账户信息并更新 LiveData
     * 4. 如果未登录，将 LiveData 设为 null
     * 5. LiveData 变化会自动通知 Activity 更新界面
     *
     * viewModelScope.launch { }：在 Main 线程启动协程，ViewModel 销毁时自动取消
     * .value =：在主线程直接更新 LiveData
     */
    fun checkLoginState() {
        viewModelScope.launch {
            ...
            if (isLoggedIn) {
                ...
                _accountInfo.value = account  // 更新 LiveData，界面会收到通知
            } else {
                // 未登录：设为 null
     ...
     * 执行流程：
     * 1. 使用 viewModelScope 启动协程（在 Main 调度器上运行）
     * 2. 调用 Repository 执行登录（Repository 内部切换到 IO 线程）
     ...
     * 执行流程：
     * 1. 使用 viewModelScope 启动协程（在 Main 调度器上运行）
     * 2. 调用 Repository 执行登出（Repository 内部切换到 IO 线程）
     ...
     * 执行流程：
     * 1. 使用 viewModelScope 启动协程（在 Main 调度器上运行）
     * 2. 调用 Repository 执行注册（Repository 内部切换到 IO 线程）
     ...
     * 执行流程：
     * 1. 使用 viewModelScope 启动协程（在 Main 调度器上运行）
     * 2. 调用 Repository 检查用户名（Repository 内部切换到 IO 线程）
     ...
     * 执行流程：
     * 1. 使用 viewModelScope 启动协程（在 Main 调度器上运行）
     * 2. 调用 Repository 更新账户信息（Repository 内部切换到 IO 线程）
    ...
    // 注意：不再需要 onCleared() 方法
    // viewModelScope 会在 ViewModel 被销毁时自动取消所有协程
    // 这是使用 viewModelScope 的优势之一：无需手动管理协程生命周期
}
