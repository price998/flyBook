...
import com.example.myapplication.data.db.account.AccountDatabase
...
class AccountRepository(context: Context) {
     ...
     * - 确保数据库操作在 IO 线程执行，避免阻塞主线程
     */
    suspend fun isUsernameExists(username: String): Boolean = withContext(Dispatchers.IO) {
        try {
     ...
     * - 确保数据库操作在 IO 线程执行，避免阻塞主线程
     */
    suspend fun updateAccount(username: String?, password: String?): Boolean = withContext(Dispatchers.IO) {
        try {
            // 获取当前登录的账户，如果为 null 则直接返回 false
            val current = accountDao.getLoggedInAccount() ?: return@withContext false
